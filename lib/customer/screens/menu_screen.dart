    ...
    try {
          ...
          menuItems =
              data.map((cake) {
                ...
                final String catName =
                    (catObj is Map) ? (catObj['name'] ?? 'General') : 'General';
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              "Backend Restricted: Please check Supabase RLS Policies",
            ),
  ...
  void _sortItems(String criteria) {
    setState(() {
      if (criteria == "Price: Low to High") {
        filteredItems.sort(
          (a, b) => a['numericPrice'].compareTo(b['numericPrice']),
        );
      } else if (criteria == "Price: High to Low") {
        filteredItems.sort(
          (a, b) => b['numericPrice'].compareTo(a['numericPrice']),
        );
        ...
        child: CustomScrollView(
          slivers: [
            ...
            if (_isLoading)
              ...
              _isGridView
                  ? _buildGrid(primaryColor, secondaryColor, onSurface)
                  : _buildList(primaryColor, secondaryColor, onSurface),
